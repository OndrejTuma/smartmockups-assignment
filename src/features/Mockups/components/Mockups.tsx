  ...
  return (
    <div className={'grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 text-none'}>
      {mockups.map(({ id, title, thumb }) => (
        <div key={id} className={'relative text-center bg-gray'}>
          <Image src={thumb} width={360} height={270}/>
          <div className={'text-base opacity-0 bg-gradient-to-t from-gray absolute inset-0 transition-all hover:opacity-100'}>
